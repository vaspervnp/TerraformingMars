# Mars Terraforming — Οδηγός Παιχνιδιού (Gameplay)

[🇬🇧 English](GAMEPLAY.md) · **🇬🇷 Ελληνικά** · [📄 Λήψη PDF](docs/GAMEPLAY.el.pdf)
...
**Το 100% είναι ο στόχος, όχι ρεκόρ προς κατάρριψη.** Μόλις ένας μετρητής περάσει τον στόχο, σταματά να δείχνει ποσοστό και δείχνει **πόσο πάνω** είσαι (π.χ. `+16,0 C`) με ένα βελάκι προς τα πάνω: πορτοκαλί σημαίνει ότι η παραπάνω παραγωγή πάει χαμένη, **κόκκινο σημαίνει κίνδυνος** — θερμοκρασία ή πίεση πάνω από +4 από τον στόχο είναι έδαφος για runaway greenhouse, όπου το πλήρωμα αρρωσταίνει, η βλάστηση μαραίνεται και οι ωκεανοί εξατμίζονται. Μια γραμμή στο κάτω μέρος της οθόνης το λέει ρητά. Κόψε ή ξήλωσε εργοστάσια αερίων και καθρέφτες πριν φτάσεις εκεί.
...
**Επισκευές:** ένα κτήριο που «έπεσε» από γεγονός, σεισμό ή καταιγίδα δεν παράγει τίποτα μέχρι να επισκευαστεί. Στην οθόνη πληρώματος τέτοια κτήρια μπαίνουν **πρώτα-πρώτα**, με κόκκινο, και έχουν **θέση συνεργείου** — ακόμη κι αυτά που κανονικά δουλεύουν μόνα τους και δεν έχουν καθόλου θέσεις εργασίας. Ρίξε εκεί έναν άποικο και η επισκευή τρέχει πιο γρήγορα: ο **Engineer επισκευάζει τρεις φορές γρηγορότερα** από το τίποτα, οποιοσδήποτε άλλος δύο. Μόλις τελειώσει η επισκευή, ο άποικος **αποδεσμεύεται μόνος του** και γυρνά στους διαθέσιμους.

**Η οθόνη πληρώματος (C):** με το **C** — ή το εικονίδιο με τους δύο αποίκους στην κάτω μπάρα — βλέπεις όλους τους αποίκους και όλες τις θέσεις εργασίας μαζί: αριστερά όσοι είναι ελεύθεροι, δεξιά όλα τα κτήρια που έχουν θέσεις. **Σέρνεις** έναν άποικο πάνω σε μια θέση για να τον μεταφέρεις εκεί· αν η θέση είναι ήδη πιασμένη, οι δύο άποικοι **ανταλλάσσουν κτήριο**. Drop στο *UNASSIGNED* τον βγάζει από τη δουλειά. Το `*` σημαίνει ότι η ειδικότητά του ταιριάζει με το κτήριο (+50% απόδοση), ενώ κάθε κτήριο δείχνει την τρέχουσα απόδοσή του — έτσι βλέπεις με μια ματιά πού αξίζει ένα ανακάτεμα.
...
Δύο τεχνολογίες δεν ξεκλειδώνουν κτήριο — ξεκλειδώνουν **έδαφος όπου δεν μπορούσες να χτίσεις**:

- **Mountain Foundations** (Φάση 2, μετά τη *Heavy Metallurgy*) — αγκυρώσεις στον βράχο και αναβαθμίδες. Τα βουνά παύουν να είναι απαγορευμένα, κάτι που μετράει επειδή ακριβώς εκεί περνούν οι φλέβες σιδήρου και πυριτίου.
- **Offshore Drill Platforms** (Φάση 3, μετά τους *Orbital Mirrors*) — όταν οι καθρέφτες λιώσουν τους πόλους, ο πάγος γίνεται θάλασσα αλλά το κοίτασμα μένει από κάτω. Οι πλωτές εξέδρες επιτρέπουν **ice drill πάνω σε νερό που κρύβει πάγο** — όλα τα άλλα κτήρια θέλουν ακόμη στεριά.

Τα κτήρια που απαιτούν συγκεκριμένο κοίτασμα το απαιτούν ούτως ή άλλως, και ο δείκτης τοποθέτησης σου λέει ποια έρευνα λείπει (π.χ. *«Mountain: needs Mountain Foundations»*).
...
- **Επόμενο χαλασμένο κτήριο:** `/` — τα χαλασμένα φοράνε **κόκκινο κλειδί** στον χάρτη και δεν παράγουν τίποτα μέχρι να επισκευαστούν
- **Αναθέσεις πληρώματος (drag & drop):** **C**
...
- **Πού μένουν τα saves** — στον φάκελο του λογαριασμού σου, ώστε να επιβιώνουν από rebuild ή επανεγκατάσταση: `%APPDATA%\Mars Terraforming\SavedGames` σε Windows, `~/.local/share/MarsTerraforming/SavedGames` σε Linux (τηρείται το `$XDG_DATA_HOME`), `~/Library/Application Support/Mars Terraforming/SavedGames` σε macOS. Η ακριβής διαδρομή γράφεται στο κάτω μέρος της οθόνης «Load Game». Όσα save έχουν μείνει δίπλα σε παλιότερο build μεταφέρονται εκεί αυτόματα στο ξεκίνημα.
...
*Αυτό είναι το Mars Terraforming.*
